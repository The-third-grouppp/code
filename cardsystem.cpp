...
bool CardSystem::initSystem()
{
    ...
    {
        cardTemplates.emplace_back("TPL001", "普通卡", 0, 10000, 1.0, 365);
        cardTemplates.emplace_back("TPL002", "银卡", 500, 20000, 0.95, 730);
        cardTemplates.emplace_back("TPL003", "金卡", 2000, 50000, 0.90, 1095);
    }
...
void CardSystem::handleCreateCard()
{
    ...
    std::cout << "请输入联系电话（可选）：";
    std::cin >> phone;
    std::cout << "请输入卡模板ID（TPL001-普通卡, TPL002-银卡, TPL003-金卡）：";
    std::cin >> templateId;
    if (templateId.empty()) templateId = "TPL001";

    if (createShoppingCard(holderName, phone, templateId))
        std::cout << "\n购物卡创建成功！卡号：" << shoppingCards.back().getCardId() << std::endl;
    else
        std::cout << "\n创建购物卡失败！" << std::endl;
}
...
void CardSystem::handleConsume()
{
    ...
    if (card)
    {
        std::cout << "当前余额：" << std::fixed << std::setprecision(2) << card->getBalance() << std::endl;
        std::cout << "请输入消费金额：";
        std::cin >> amount;
        std::cout << "请输入备注（可选）：";
        std::cin >> description;
        if (card->consume(amount, description))
        {
            std::cout << "\n消费成功！当前余额：" << std::fixed << std::setprecision(2) << card->getBalance() << std::endl;
            std::vector<Record> cardRecords = card->getRecords();
            if (!cardRecords.empty())
                addRecord(cardRecords.back());
        }
        else
            std::cout << "\n消费失败！余额不足或卡片状态异常" << std::endl;
    }
    else
    {
        std::cout << "\n未找到卡号：" << cardId << std::endl;
...
void CardSystem::handleRefund()
{
    ...
    if (card)
    {
        ...
        std::cout << "请输入备注（可选）：";
        std::cin >> description;
        if (card->refund(amount, description))
        {
            std::cout << "\n退款成功！当前余额：" << std::fixed << std::setprecision(2) << card->getBalance() << std::endl;
            std::vector<Record> cardRecords = card->getRecords();
            if (!cardRecords.empty())
                addRecord(cardRecords.back());
        }
        else
            std::cout << "\n退款失败！卡片状态异常或金额无效" << std::endl;
    }
    else
    {
        std::cout << "\n未找到卡号：" << cardId << std::endl;
